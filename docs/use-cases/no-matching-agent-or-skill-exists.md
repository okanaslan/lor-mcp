...
A Codex agent asks Local Orchestration Router (LOR) for a relevant agent or
skill, but no catalog entry matches the task.
...
agent or skill. The active catalog does not contain a relevant entry.
...
3. Local Orchestration Router (LOR) searches introduced agents and skills in the
...
or skill, and the user can decide whether to introduce a new catalog entry
later.
...
- Should Local Orchestration Router (LOR) suggest metadata for a future agent or
  skill when no match exists?
