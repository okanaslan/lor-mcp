...
Implemented for the public V2 surface through skill and subagent listing,
including shared global skills and global subagents by default.

Current public MCP tools are `list_skills` and `list_subagents`;
registered-agent listing and the generic `list_catalog_entries` tool name are no
longer registered in the normal public V2 surface.

## 2. Goals

- Show what skills and subagent profiles have been introduced in the workspace
  catalog.
- Show shared global skills alongside workspace-local entries by default.
- Show workspace-local and global subagents alongside skills.
- Support simple filtering by project and scope where available.
...
which skills and subagents are currently available before asking for routing.
...
- 2026-08-15: Remove registered-agent listing from the normal public V2 surface.
