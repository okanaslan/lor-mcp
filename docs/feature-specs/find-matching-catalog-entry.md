...
Implemented for the public V2 surface through skill and subagent prompt profile
matching, including shared global skills and global subagents by default.

Current public MCP tools are `find_matching_skill` and `find_matching_subagent`;
registered-agent matching and the generic `find_matching_catalog_entry` tool
name are no longer registered in the normal public V2 surface.

## 2. Goals

- Match task intent against introduced skills and subagent profiles.
...
- Return enough next-step guidance for callers to use returned context safely.

## 3. Non-Goals

- Introduce new skills or subagent profiles.
- Modify catalog entries.
- Define final ranking weights for every possible specialty.
- Search registered-agent entries.
...
- The server must return ranked matching skills and subagents when relevant
  entries exist.
...
- Skills and subagents must remain ranked results and must not create
  registered-agent conflict results.
...
Optional for later expansion. The main V2 use case is that a user asks which
registered skills and subagent profiles should shape a task, and Local
Orchestration Router (LOR) returns ranked context with explanation metadata.
...
- `entryType`: identifies whether the result is a skill or subagent.
...
- No-match results must be returned as structured non-error outcomes.
...
- 2026-07-11: Match both introduced agents and introduced skills in the initial
  catalog design.
