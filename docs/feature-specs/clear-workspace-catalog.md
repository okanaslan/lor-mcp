...
Implemented for the public V2 surface through skill and subagent clear tools.
This feature lets a caller clear introduced workspace-local catalog entries from
one requested workspace.

Current public MCP tools are `clear_workspace_skills` and
`clear_workspace_subagents`; registered-agent clearing and the generic
`clear_workspace_catalog` tool name are no longer registered in the normal
public V2 surface.

## 2. Goals

- Let users reset workspace-local skill and subagent catalog records without
  deleting local skill files or Codex tasks.
...
- Support clearing skills or subagent profiles through explicit typed tools.
...
- The tool names are `clear_workspace_skills` and `clear_workspace_subagents`.
...
`clear_workspace_skills` or `clear_workspace_subagents` with the current
workspace and explicit confirmation, then re-register the desired skills and
subagent profiles.
...
- 2026-08-15: Remove registered-agent clearing from the normal public V2
  surface.
